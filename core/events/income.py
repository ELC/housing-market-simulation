...
class AgentIncomeReceived(Event):
    ...
    def apply(self, market: "HousingMarket", context: "SimulationContext") -> ApplyResult[Self]:
        ...
        gross, net = self.compute_salary(agent)
        next_event = self.model_copy(update={
            "time": self.time + 1,
            "amount": net,
            "gross_income": gross,
        })
